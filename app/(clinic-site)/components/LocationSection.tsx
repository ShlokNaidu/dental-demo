"use client";

import React from "react";
import { ScrollReveal } from "@/components/animations/ScrollReveal";
import { CLINIC_ADDRESS, CLINIC_HOURS, CLINIC_PHONE } from "@/lib/utils/constants";

const INFO_ROWS = [
  {
    label: "Address",
    value: CLINIC_ADDRESS,
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
    ),
  },
  {
    label: "Clinic Hours",
    value: CLINIC_HOURS,
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
    ),
  },
  {
    label: "Phone & WhatsApp",
    value: CLINIC_PHONE,
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
    ),
  },
];

export const LocationSection: React.FC = () => {
  return (
    <section
      id="location"
      className="py-24"
      style={{ background: "var(--bg-base)" }}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <ScrollReveal className="mb-16">
          <p className="label-caps text-teal mb-3">Visit Our Clinic</p>
          <h2 className="text-display-md text-ivory">Location & Hours</h2>
        </ScrollReveal>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          {/* Info — left */}
          <ScrollReveal direction="left" delay={0.1} className="lg:col-span-5">
            <div
              className="rounded-2xl p-8 h-full space-y-0"
              style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-subtle)" }}
            >
              {INFO_ROWS.map((row, i) => (
                <div
                  key={row.label}
                  className="flex items-start gap-5 py-6"
                  style={{ borderBottom: i < INFO_ROWS.length - 1 ? "1px solid var(--border-subtle)" : "none" }}
                >
                  <div
                    className="p-3 rounded-xl flex-shrink-0"
                    style={{ background: "var(--teal-dim)", color: "var(--teal)" }}
                  >
                    {row.icon}
                  </div>
                  <div>
                    <h3 className="font-bold text-ivory text-base mb-1" style={{ fontFamily: "'Hanken Grotesk', sans-serif" }}>
                      {row.label}
                    </h3>
                    <p className="text-sm" style={{ color: "var(--stone-400)" }}>{row.value}</p>
                  </div>
                </div>
              ))}

              {/* AI badge */}
              <div
                className="mt-4 p-4 rounded-xl flex items-center gap-3 text-sm"
                style={{ background: "rgba(13,148,136,0.1)", border: "1px solid rgba(13,148,136,0.25)", color: "var(--teal)" }}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="flex-shrink-0"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z"/></svg>
                <span className="font-medium">WhatsApp AI Assistant active 24/7 for automated booking.</span>
              </div>
            </div>
          </ScrollReveal>

          {/* Map — right */}
          <ScrollReveal direction="right" delay={0.2} className="lg:col-span-7">
            <div className="w-full h-full min-h-96 rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border-subtle)" }}>
              <iframe
                title="Smile Care Dental Clinic Vijay Nagar Indore Map"
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d14717.391662584105!2d75.8856!3d22.7533!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x396302af403406fb%3A0x5b3310065095368a!2sVijay%20Nagar%2C%20Indore%2C%20Madhya%20Pradesh!5e0!3m2!1sen!2sin!4v1700000000000!5m2!1sen!2sin"
                width="100%"
                height="100%"
                style={{ border: 0, minHeight: "384px", filter: "grayscale(20%) contrast(1.05)" }}
                allowFullScreen={false}
                loading="lazy"
                referrerPolicy="no-referrer-when-downgrade"
              />
            </div>
          </ScrollReveal>
        </div>
      </div>
    </section>
  );
};
